...
- Can we do better than guessing? To answer this question we need a quantitative definition of **better**. 
...
- Note: statisticians call this a _contingency table_
...
Specificity | True negative rate (TNR) |1-FPR | $TN / (TN+FP)$ | $\mbox{Pr}(\hat{Y}=0 \mid Y=0)$ |
Specificity | Positive Predictive value | Precision | $TP / (TP+FP)$ | $\mbox{Pr}(Y=1 \mid \hat{Y}=1)$|
